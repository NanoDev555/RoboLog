...
- [Arduino Code](#arduino-code)
  - [Arduino Code Purpose](#arduino-code-purpose)
  - [Arduino Code Key Functionality](#arduino-code-key-functionality)
    - [Initialization](#arduino-initialization)
    - [Main Loop](#arduino-main-loop)
    - [Data Transmission](#arduino-data-transmission)
- [Python Code](#python-code)
  - [Python Code Purpose](#python-code-purpose)
  - [Python Code Key Functionality](#python-code-key-functionality)
    - [Initialization](#python-initialization)
    - [Main Loop](#python-main-loop)
    - [Supporting Functions](#python-supporting-functions)
...
### Arduino Code Key Functionality

#### Arduino Initialization

- **Setup Function (`setup()`)**:
  - Initializes serial communication at 115200 baud.
  - Initializes arrays for averaging IMU readings.
  - Sets pin modes for the potentiometer (`potPin3`) and gripper button (`buttonGripperPin`).
  - Initializes the IMU and checks for successful startup.

#### Arduino Main Loop

- **Non-blocking Timing**:
  - Uses `millis()` function to implement a non-blocking delay, allowing sensor sampling at regular intervals without halting other processes.

- **Sensor Reading and Averaging**:
  - Reads IMU acceleration data along the X and Y axes.
  - Stores readings in arrays and calculates moving averages to smooth out noise.

- **Change Detection**:
  - Compares current average readings with the last recorded averages.
  - Checks if the change exceeds predefined thresholds (`imuThreshold` for IMU, `inputThreshold` for potentiometer).

- **Gripper Button Handling**:
  - Implements debounce logic to prevent false triggers due to button bounce.
  - Toggles the `gripperState` when a valid button press is detected.

- **Data Transmission Trigger**:
  - Only sends data when a significant change in sensor readings or gripper state is detected.

#### Arduino Data Transmission

- **`printData()` Function**:
  - Formats and sends sensor data and gripper state over serial communication.
  - Data format:
    ```
